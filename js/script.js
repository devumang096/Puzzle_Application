...
function generateMaze(rows, cols) {
  ...
  generateMazeRecursively(startRow, startCol, grid);
  // Make sure the maze meets the minimum requirements
  var numClues = countClues(grid);
  var numDeadEnds = countDeadEnds(grid);
  var numSolutions = countSolutions(grid);
  while (numClues < 5 || numDeadEnds < 2 || numSolutions < 1) {
    grid = initializeGrid(rows, cols);
    startRow = Math.floor(Math.random() * rows);
    startCol = Math.floor(Math.random() * cols);
    generateMazeRecursively(startRow, startCol, grid);
    numClues = countClues(grid);
    numDeadEnds = countDeadEnds(grid);
    numSolutions = countSolutions(grid);
  }
...
function countSolutions(grid) {
  ...
  var visited = initializeVisited(grid);
  var numSolutions = countSolutionsRecursively(startCell.row, startCell.col, visited, grid);
  return numSolutions;
}

function countSolutionsRecursively(row, col, visited, grid) {
  if (isExit(row, col, grid)) {
    return 1;
  }
  ...
  for (var i = 0; i < DIRECTIONS.length; i++) {
    ...
    if (isValidCell(newRow, newCol, grid) && !visited[newRow][newCol] && !isWall(newRow, newCol, grid)) {
      numSolutions += countSolutionsRecursively(newRow, newCol, visited, grid);
    }
  }
